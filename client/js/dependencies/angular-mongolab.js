angular.module('mongolabResourceHttp', [])
    ...
    function MmongolabResourceFactory(collectionName) {
        ...
        //Resource.prototype.$remove didn´t work for me, this is a quick hack
        //upd: 3 days later. ok, now I know how instance methods work.
        /* like this:
         new Resource(document).$remove().then(function (data) {
                                console.log(data);
                            });
        */
        Resource.remove_one = function (id) {
            
            if (id && id.$oid) {
            return $http['delete'](collectionUrl + "/" + id.$oid + "?apiKey="+defaultParams.apiKey);
                       } else if (id) {
            return $http['delete'](collectionUrl + "/" + id + "?apiKey="+defaultParams.apiKey);
                       }
        };
